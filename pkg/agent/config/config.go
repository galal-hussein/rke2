...
import (
	...
	"github.com/rancher/rke2/pkg/containerd"
...
// server.
func Get(ctx context.Context, agent *config.Agent, proxy proxy.Proxy) *config.Node {
...
func locateOrGenerateResolvConf(envInfo *config.Agent) string {
...
func get(ctx context.Context, envInfo *config.Agent, proxy proxy.Proxy) (*config.Node, error) {
	if envInfo.Debug {
	...
	nodeConfig.AgentConfig.ClusterDNS = controlConfig.ClusterDNSIP
	...
	nodeConfig.AgentConfig.NodeIPStr = nodeIP.String()
	...
	if len(nodeConfig.AgentConfig.NodeExternalIPs) > 0 {
		...
		nodeConfig.AgentConfig.NodeExternalIPStr = nodeExternalIP.String()
	}
	...
	if len(controlConfig.ClusterDNSIPs) == 0 {
		nodeConfig.AgentConfig.ClusterDNSs = []net.IP{controlConfig.ClusterDNSIP}
	} else {
		nodeConfig.AgentConfig.ClusterDNSs = controlConfig.ClusterDNSIPs
	}
...
func getConfig(info *clientaccess.Info) (*config.Server, error) {
	data, err := info.Get("/v1-" + version.Program + "/config")
	if err != nil {
		return nil, err
	}

	controlControl := &config.Server{}
