...
func Server(ctx context.Context, cfg *config.Server) error {
...
func controllerManager(ctx context.Context, cfg *config.Server) error {
	runtime := cfg.Runtime
...
func scheduler(ctx context.Context, cfg *config.Server) error {
	runtime := cfg.Runtime
	argsMap := map[string]string{
	...
	args := config.GetArgs(argsMap, cfg.ExtraSchedulerArgs)

	logrus.Infof("Running kube-scheduler %s", config.ArgString(args))
	return executor.Scheduler(ctx, cfg.Runtime.APIServerReady, args)
}

func apiServer(ctx context.Context, cfg *config.Server) error {
	runtime := cfg.Runtime
	argsMap := map[string]string{
		"feature-gates": "JobTrackingWithFinalizers=true",
	}
...
func defaults(config *config.Server) {
	...
	if len(config.ClusterDNS) == 0 {
		config.ClusterDNSIP = net.ParseIP("10.43.0.10")
	}
...
func prepare(ctx context.Context, config *config.Server) error {
	var err error
...
func setupStorageBackend(argsMap map[string]string, cfg *config.Server) {
...
func cloudControllerManager(ctx context.Context, cfg *config.Server) error {
	runtime := cfg.Runtime
	argsMap := map[string]string{
		"profiling":                    "false",
		"allocate-node-cidrs":          "true",
