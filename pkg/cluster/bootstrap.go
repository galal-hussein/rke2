...
func (c *Cluster) Bootstrap(ctx context.Context, snapshot bool) error {
	...
	if c.managedDB != nil {
		if !snapshot {
			isHTTP := c.config.ServerURL != "" && c.config.Token != ""
	...
	if c.managedDB != nil {
		...
		if isInitialized {
			...
			if c.config.ServerURL != "" && c.config.Token != "" {
				c.clientAccessInfo, _ = clientaccess.ParseAndValidateTokenForUser(c.config.ServerURL, c.config.Token, "server")
			}
			return false, true, nil
		} else if c.config.ServerURL == "" {
			...
			info, err := clientaccess.ParseAndValidateTokenForUser(c.config.ServerURL, c.config.Token, "server")
			if err != nil {
				return false, false, err
			}
...
func (c *Cluster) Snapshot(ctx context.Context, config *config.Server) error {
	if c.managedDB == nil {
...
func (c *Cluster) compareConfig() error {
	...
	agentClientAccessInfo, err := clientaccess.ParseAndValidateTokenForUser(c.config.ServerURL, token, "node")
	if err != nil {
		return err
	}
