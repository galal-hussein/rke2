...
type Driver interface {
	IsInitialized(ctx context.Context, config *config.Server) (bool, error)
	Register(ctx context.Context, config *config.Server, handler http.Handler) (http.Handler, error)
	...
	EndpointName() string
	Snapshot(ctx context.Context, config *config.Server) error
