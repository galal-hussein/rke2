...
func encryptionStatusHandler(server *config.Server) http.Handler {
...
func encryptionStatus(server *config.Server) (EncryptionState, error) {
...
func encryptionEnable(ctx context.Context, server *config.Server, enable bool) error {
	providers, err := secretsencrypt.GetEncryptionProviders(server.Runtime)
	if err != nil {
		return err
	}
...
func encryptionPrepare(ctx context.Context, server *config.Server, force bool) error {
